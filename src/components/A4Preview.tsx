...
export const A4Preview: React.FC<A4PreviewProps> = ({ state, indexRows, activeTab, id = 'a4-print-page' }) => {
  ...
  // Force colors into a standard Hex format, as html2canvas fails on oklch()
    ...
    <div className="flex-1 flex justify-center items-start overflow-auto p-4 md:p-8 select-none">
      ...
      >
        <div className="w-full h-full flex flex-col box-border relative" style={{ padding: bs.padding }}>
          <div className="w-full h-full flex flex-col box-border relative" style={{ border: `${bs.thickness} solid ${forceHex(bs.borderColor)}`, padding: bs.gap }}>
            <div className="w-full h-full flex flex-col items-center box-border relative" style={{ border: `${bs.thickness} solid ${forceHex(bs.borderColor)}`, padding: '2.5rem 1.8rem' }}>
